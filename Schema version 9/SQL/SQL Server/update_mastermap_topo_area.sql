UPDATE [dbo].[OS_MASTERMAP_TOPO_AREA] SET STYLE_DESCRIPTION = 
 ...
 WHEN CHARINDEX('Nonconiferous Trees', DescriptiveTerm) > 0 OR CHARINDEX('Nonconiferous Trees (Scattered)', DescriptiveTerm) > 0 THEN 'Nonconiferous Tree Fill'  WHEN CHARINDEX('Coniferous Trees', DescriptiveTerm) > 0 OR CHARINDEX('Coniferous Trees (Scattered)', DescriptiveTerm) > 0 THEN 'Coniferous Tree Fill'  
 ...
 WHEN CHARINDEX('Inland Water', DescriptiveGroup) > 0 THEN 'Inland Water Fill'  WHEN CHARINDEX('Path', DescriptiveGroup) > 0 THEN 'Path Fill'  
